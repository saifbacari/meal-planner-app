...
**Statut** : MVP implémenté — en test & itération
**Plateforme** : iOS/Android (React Native via Expo)
...
- **State Management** : Context API (Auth, Fridge, Favorites, Preferences)
- **Navigation** : Expo Router v3 (file-based)
- **UI** : Custom design system dark theme (`constants/theme.ts`)

### Backend
- **Database & Auth** : Supabase (PostgreSQL + Auth)
  - Table `user_preferences` avec RLS
  - Cascade delete sur suppression de compte
- **API IA** : Claude API `claude-haiku-4-5-20251001` (appelée directement depuis le client)
  - Pas de proxy Vercel — appel direct avec clé en `EXPO_PUBLIC_*`
- **API Recettes** : Aucune API externe — recettes 100% générées par Claude

### Infrastructure
- **Hosting App** : À déployer (Expo EAS Build prévu)
- **CI/CD** : Non configuré
- **Gestion projet** : Linear (via MCP server)
...
- Session persistence via AsyncStorage (SecureStore retiré — limite 2048 bytes)
- Logout + suppression de compte (Supabase RPC `delete_user`)
- Password peek à la saisie (600ms)

---

#### ✅ **2. Onboarding**
**Status** : ✅ DONE
- Welcome screen + 5 steps + finish screen avec animation
- Step 1 : Objectifs (lose_weight, eat_healthy, build_muscle, maintain)
- Step 2 : Régime alimentaire (multiple) + Allergies
- Step 3 : Temps préparation + Niveau cuisine
- Step 4 : Ingrédients fréquents (pré-remplit le frigo)
- Step 5 : Équipement cuisine (four, micro-ondes, air fryer, blender, vapeur, cocotte-minute)
- Données sauvegardées en Supabase (`user_preferences`) + cache AsyncStorage

---

#### ✅ **3. Dashboard - Suggestion Engine**
**Status** : ✅ DONE
- Chips forme physique + envies du moment
- Suggestions IA (Claude) basées sur le frigo + préférences alimentaires
- Filtrage local par état physique/envies
- Pills "Filtré selon" avec les préférences actives
- Empty / Loading / Error states implémentés
- RecipeDetailModal avec étapes générées à la demande

---

#### ✅ **4. Suggestions IA Claude**
**Status** : ✅ DONE (approche différente de la spec initiale)
- 100% Claude API — pas d'API recettes externe
- Préférences injectées dans le prompt (régime, allergies, objectifs, niveau, temps, équipement)
- Équipement utilisateur contraint côté prompt : seules les recettes réalisables avec l'équipement disponible sont proposées
- Ingrédients marqués disponible/manquant selon le contenu du frigo (`RecipeIngredient { name, available }`)
- Recettes pouvant inclure des ingrédients absents du frigo (affichés en orange avec icône panier)
- Contraintes temps/état physique (tired ≤20min, sleep ≤15min, post_sport ≤30min)
- Post-processing client pour garantir la cohérence
